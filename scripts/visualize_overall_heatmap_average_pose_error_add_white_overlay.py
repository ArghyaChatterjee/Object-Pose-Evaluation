...
def make_scene_mesh(obj_data, colors_rgba, translation):
    ...
    # Center each mesh on its own bounding-box center.
    center = 0.5 * (vertices_m.min(axis=0) + vertices_m.max(axis=0))
    vertices = vertices_m - center

    # Rotate for a nice 3D view.
    R_view = rotation_matrix_xyz(
        VIEW_ROT_X_DEG,
        VIEW_ROT_Y_DEG,
        VIEW_ROT_Z_DEG,
    )
    vertices = (R_view @ vertices.T).T

    vertices *= VISUAL_SCALE

    # Put into shared scene.
    vertices[:, 0] += translation[0]
    vertices[:, 1] += translation[1]
    vertices[:, 2] += translation[2]

    # OpenCV-ish scene to OpenGL camera convention.
...
def layout_positions(n_objects):
    # Similar to the sample image: objects laid out horizontally in rows.
    # 12 objects -> 4 columns x 3 rows.
    n_cols = 6
