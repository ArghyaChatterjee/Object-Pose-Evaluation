...
OUTPUT_DIR = DATA_ROOT / "results" / "overall_sequence_add_heatmap" / OBJECT_NAME
OUTPUT_IMAGE = OUTPUT_DIR / f"{OBJECT_NAME}_overall_sequence_add_heatmap_first_gt_view_colorbar.png"
OUTPUT_SUMMARY = OUTPUT_DIR / f"{OBJECT_NAME}_overall_sequence_add_summary.txt"
...
# "mean" = average per-vertex error over sequence
# "sum"  = total accumulated per-vertex error over sequence
...
def compute_sequence_vertex_errors(vertices_m):
    ...
    for gt_path in gt_paths:
        ...
        per_vertex_errors = np.linalg.norm(pts_est - pts_gt, axis=1)

        sum_vertex_errors += per_vertex_errors

        frame_add = float(np.mean(per_vertex_errors))
        frame_add_values.append(frame_add)
        valid_frames += 1

        print(
            f"[FRAME] {gt_path.stem}: "
            f"ADD = {frame_add:.8f} m = {frame_add * 100.0:.4f} cm"
        )
    ...
    return {
        ...
        "frame_add_values": np.asarray(frame_add_values, dtype=np.float64),
...
def print_distribution(stats):
    print("\n[OVERALL SEQUENCE COLOR DISTRIBUTION]")
...
def save_with_colorbar(render_rgb, stats, out_path, first_frame_name):
    ...
    if ACCUMULATION_MODE == "mean":
        cbar_label = "Mean per-vertex ADD over sequence (cm)"
        title_metric = "Overall mean ADD heatmap"
    else:
        cbar_label = "Summed per-vertex ADD over sequence (cm)"
        title_metric = "Overall summed ADD heatmap"
...
def main():
    ...
    result = compute_sequence_vertex_errors(vertices_m)

    final_vertex_errors = result["final_vertex_errors"]
    frame_add_values = result["frame_add_values"]
    ...
    add_recall = float(np.mean(frame_add_values < threshold)) * 100.0
    sequence_mean_add = float(np.mean(frame_add_values))
    ...
    with open(OUTPUT_SUMMARY, "w") as f:
        ...
        f.write(f"add_0.1d_threshold_m: {threshold:.10f}\n")
        f.write(f"add_0.1d_recall_percent: {add_recall:.6f}\n")
        f.write(f"sequence_mean_add_m: {sequence_mean_add:.10f}\n")
        f.write(f"sequence_mean_add_cm: {sequence_mean_add * 100.0:.10f}\n")
    ...
    print(f"[INFO] ADD-0.1d threshold: {threshold:.8f} m")
    print(f"[INFO] ADD-0.1d Recall: {add_recall:.4f} %")
    print(
        f"[INFO] Sequence mean ADD: "
        f"{sequence_mean_add:.8f} m = {sequence_mean_add * 100.0:.4f} cm"
    )
