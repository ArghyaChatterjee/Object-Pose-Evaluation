...
OUTPUT_DIR = DATA_ROOT / "results" / "add_heatmap_white_dynamic" / DATASET_NAME
...
# False: min error -> lowest color, max error -> highest color.
# True:  0 error   -> lowest color, max error -> highest color.
...
def print_stats(stem, stats):
    ...
    print(f"  mean ADD         : {stats['err_mean']:.8f} m = {stats['err_mean'] * 100:.4f} cm")
...
def save_with_colorbar(render_rgb, stats, out_path):
    """
    Saves render with a vertical colorbar on the right.
    Colorbar units are centimeters.
    """
    ...
    cbar.set_label("ADD vertex error (cm)", rotation=90, labelpad=15)

    title = (
        f"Dynamic heatmap: "
        f"{norm_min_cm:.3f} cm to {norm_max_cm:.3f} cm\n"
        f"Mean ADD: {stats['err_mean'] * 100.0:.3f} cm"
    )
...
def render_one_frame(rasterizer, mesh_template, gt_json, est_json, raw_out_path, colorbar_out_path):
    stem = gt_json.stem
    ...
    errors_m = np.linalg.norm(pts_gt_cv_m - pts_est_cv_m, axis=1)

    colors_rgba, stats = error_to_colors_dynamic(errors_m)
    print_stats(stem, stats)

    # Render GT mesh, colored by predicted-pose error.
    ...
    if SAVE_RAW_RENDER:
        cv2.imwrite(str(raw_out_path), cv2.cvtColor(color, cv2.COLOR_RGB2BGR))
...
def main():
    ...
    print(f"[INFO] Save colorbar: {SAVE_WITH_COLORBAR}")
    ...
    csv_path = OUTPUT_DIR / "per_frame_error_distribution.csv"
    ...
    with open(csv_path, "w") as f:
        ...
        for gt_json in gt_jsons:
            ...
            raw_out_path = OUTPUT_DIR / f"{stem}_white_dynamic_heatmap.png"
            colorbar_out_path = OUTPUT_DIR / f"{stem}_white_dynamic_heatmap_colorbar.png"
            ...
            mean_add_values.append(stats["err_mean"])
            ...
            if SAVE_RAW_RENDER:
                print(f"[INFO] Saved raw render: {raw_out_path}")
            if SAVE_WITH_COLORBAR:
                print(f"[INFO] Saved colorbar render: {colorbar_out_path}")

    if len(mean_add_values) > 0:
        summary_path = OUTPUT_DIR / "summary.txt"

        with open(summary_path, "w") as f:
            f.write(f"dataset: {DATASET_NAME}\n")
            f.write(f"frames_rendered: {len(mean_add_values)}\n")
            ...
            f.write(f"sequence_mean_add_m: {np.mean(mean_add_values):.10f}\n")
            f.write(f"sequence_mean_add_cm: {np.mean(mean_add_values) * 100:.10f}\n")
        ...
        print(f"\n[INFO] Rendered frames: {len(mean_add_values)}")
        print(f"[INFO] Sequence mean ADD: {np.mean(mean_add_values):.8f} m = {np.mean(mean_add_values) * 100:.4f} cm")
